...
To connect to Safe/GEMRIC (Henderson server): Establish a VPN connection using Cisco AnyConnect (profile UiB Safe, setup at https://vpn3.uib.no), then use Remote Desktop Protocol (RDP) to connect to henderson.uib.no. On Windows machines use the application "Remote Desktop Connection" for MacOS the application is now called "Windows App" (see app store). For image data analysis purposes, connect to Henderson first and from Henderson connect to Homlungen.uib.no using RDP (homlungen.uib.no:52525/). All connections (VPN, RDP, ssh, REDCap) use your UiB credentials for authentication.
...
Mount either `uib-san1-nas` or `klient.uib.no` to your local computer (as described in "To upload data"). Files that you want to download need to be placed in the Export folder on the S-drive of Henderson.uib.no. Only a GEMRIC administrator can do this operation - after a review of the files. Copy your export request files into another folder on `S:\Project\Export\<UiB_username>\` and send an email to the GEMRIC adminstrator to request the export. The GEMRIC administrator will copy your files to the `To_export`. Each file is individually compressed and secured with a password. The password is added to the `O:\Export\_password.txt` file on Henderson that contains a history of all automatically generated passwords used in the past. Use a program like 7zip to uncompress if you encounter problems with the downloaded file.
...
To setup docker images:
- **Pull docker image locally**: Install docker and pull the desired docker image on your local machine (outside Safe) using the command `docker pull <image_name>`. Make sure you are on an x86_64 machine to ensure compatibility with the homlungen server.
- **Save docker image**: Save the pulled docker image as a .tar.gz compressed file using the command `docker save <image_name> | gzip > <image_name>.tar.gz`.
- **Upload docker image to homlungen**: Connect to the homlungen server (see smb instructions) and upload the tar.gz file to a directory of your choice (e.g., `/data/transfer/`).
- **Load docker image on homlungen**: Load the docker image on homlungen using `docker load < /data/transfer/<image_name>.tar.gz`. You will need docker permissions to load and run container images. See if you have permission (try `group`, see if you have 'docker' listed). If you do not have permissions to run docker commands, contact the GEMRIC coordinators to request access. 

Docker allows for flexibility in using different software tools for data analysis while ensuring that they are compatible with the server environment. Request access to docker on the homlungen server by contacting the GEMRIC coordinators. Such requests are added by the coordinators to a shared document with the IT department of the University of Bergen. The IT department will implement the changes needed to allow access.

Homlungen does not provide sbatch as a job scheduling software (used for high-performance computing clusters). It does allow for controlled parallel job monitoring and scheduling using `gnu parallel`. Add all compute jobs into a single command file (one line, one command) and use `parallel -j 20 :::: jobs.txt` to execute up to 20 jobs at the same time. Parallel will start new jobs as soon as one of the running jobs finish. See also more advanced usages with command line options `--eta`, `--dry-run`, and `--results outdir`.
...
3. **Organize Image Data**: Once your data is pseudonymized, and packaged into zip files for each subject and session (SubjectXX_T01.zip) upload them using the mounted share (place into Henderson/Import/). On Henderson navigate to your site folder (provided by GEMRIC coordinators) and move your data to the appropriate SiteXX directory. Ensure that your data is organized according to the GEMRIC guidelines, with clear naming conventions for subjects and sessions.
