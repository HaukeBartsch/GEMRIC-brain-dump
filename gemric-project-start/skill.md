...
Homlungen does not provide sbatch as a job scheduling software (used for high-performance computing clusters). It does allow for controlled parallel job monitoring and scheduling using `gnu parallel`. Add all compute jobs into a single command file (one line one command) and use `parallel` to execute up to 20 jobs at the same time. Parallel will start new jobs from the command file as soon as one of the running jobs is finished.
