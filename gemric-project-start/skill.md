...
Mount either `uib-san1-nas` or `klient.uib.no` to your local computer (as described in "To upload data"). Files that you want to download need to be placed in the Export folder on the S-drive of Henderson.uib.no. Only a GEMRIC administrator can do this operation - after a review of the files. Copy your export request files into another folder on `S:\Project\Export\<UiB_username>\` and send an email to the GEMRIC adminstrator to request the export. The GEMRIC administrator will copy your files to the `To_export`. Each file is individually compressed and secured with a password. The password is added to the `O:\Export\_password.txt` file on Henderson that contains a history of all automatically generated passwords used in the past.
